...
describe('CountryHandlers', () => {
    ...
    test('Countries: POST followed by GET returns the same data', async () => {
        const putResponse = await request(app).post('/api/v1/countries').send({name: "Sweden"}).set('Cookie', cookie);
        await request(app).post('/api/v1/countries').send({name: "Norge"}).set('Cookie', cookie);
        expect(putResponse.status).toBe(201);

        const getResponse = await request(app).get('/api/v1/countries').set('Cookie', cookie);
    ...
    test('Countries: delete an existing country', async () => {
        const putResponse1 = await request(app).post('/api/v1/countries').send({name: "Sverige"}).set('Cookie', cookie);
        expect(putResponse1.status).toBe(201);
        const putResponse2 = await request(app).post('/api/v1/countries').send({name: "Norge"}).set('Cookie', cookie);
        expect(putResponse2.status).toBe(201);

        const deleteResponse = await request(app).delete('/api/v1/countries/1').set('Cookie', cookie);
